...
int main(int argc, char *argv[]) {
  ...
  if (runfiles == nullptr) {
    fmt::println("failed to create Runfiles");
    return 1;
  }
  ...
  // Load bunny texture
  auto texturePath = runfiles->Rlocation("mono/tutorial/wabbit_alpha.png");
  Texture2D texBunny = LoadTexture(texturePath.c_str());
      ...
      for (int i = 0; i < 100; i++) {
        if (bunniesCount < MAX_BUNNIES) {
          ...
          bunnies[bunniesCount].color =
              Color{(unsigned char)GetRandomValue(50, 240),
